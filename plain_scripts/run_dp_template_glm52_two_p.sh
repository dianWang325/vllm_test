#!/usr/bin/env bash
# Called by launch_online_glm52_two_p.py for one local vLLM process.
set -euo pipefail
...
topology='{"prefill":{"dp_size":1,"tp_size":16,"pp_size":2},"decode":{"dp_size":1,"tp_size":16,"pp_size":1}}'
...
case "$role" in
  prefill)
    ...
    p_scheduler_config='{"scheduler_config":{"profiling_chunk_config":{"enabled":true,"smooth_factor":1,"need_timing":true},"short_request_first_config":{"enabled":true,"threshold":65546,"long_max_wait_ms":2000}}}'
    exec vllm serve "$model" \
      ...
      --additional-config "$p_scheduler_config" \
