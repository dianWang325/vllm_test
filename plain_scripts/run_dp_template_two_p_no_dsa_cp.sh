#!/usr/bin/env bash
# Called by launch_online_dp_two_p_no_dsa_cp.py for one local vLLM process.
set -euo pipefail
...
case "$role" in
  prefill)
    ...
    if [[ "$node_rank" == 0 ]]; then
      dp_node_args=(--data-parallel-size-local 2 --data-parallel-start-rank 0 --nnodes 2 --node-rank 0)
    else
      dp_node_args=(--data-parallel-size-local 1 --data-parallel-rank "$dp_rank" --nnodes 4 --node-rank "$node_rank")
    fi
    case "${PD_P_SCHEDULER_MODE:-enhanced}" in
      baseline)
        scheduler_args=(--additional-config '{}')
        ;;
      enhanced)
        scheduler_args=(--additional-config '{"scheduler_config":{"profiling_chunk_config":{"enabled":true,"smooth_factor":1,"need_timing":true},"short_request_first_config":{"enabled":true,"threshold":65546,"long_max_wait_ms":2000}}}')
        ;;
      *)
