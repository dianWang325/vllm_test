#!/usr/bin/env bash
# GLM-5.2 TP16/PP2/EP on 80.5.17.108 (PP0) and 80.5.17.111 (PP1).
# CPP (profiling_chunk_config) is explicitly disabled; vLLM chunked prefill is unchanged.
set -euo pipefail

role="${1:?expected head or worker}"
case "$role" in
  head)
    local_ip=80.5.17.108
    node_rank=0
    extra_args=()
    ;;
  worker)
...
log_dir=/home/w00985415/pp2_glm_ep_108_111_85k_cpp_off_20260916
...
printf 'role=%s host=%s peer=%s max_model_len=87040 cpp_enabled=false started=%s\n' "$role" "$local_ip" "$(if [ "$role" = head ]; then echo 80.5.17.111; else echo 80.5.17.108; fi)" "$(date -Is)" > "$log_dir/${role}.meta"
