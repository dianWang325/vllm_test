...
def read_run(index: int, mode: str, dataset: str) -> dict:
    ...
    result = {
        ...
        "mode": mode,
        "dataset": dataset,
        "p0_host": status.get("p0_host", MANIFEST["p0_host"]),
        "p1_host": status.get("p1_host", MANIFEST["p1_host"]),
...
def main() -> None:
    ...
    hosts = "、".join(
        f"P0 {p0} / P1 {p1} / D {d}"
        for p0, p1, d in sorted({(row["p0_host"], row["p1_host"], row["d_host"]) for row in runs})
    )
    lines = [
        "# DeepSeek-V4-Flash A3 PD 对比实验",
        "",
        f"主机：{hosts}。模型：`/mnt/weight/DeepSeek-V4-Flash-w8a8-mtp`。三机的 `vllm_test` 均为 `back136` 提交 `{MANIFEST['commit']}`。",
        "",
        "P 跨两机为 DP2/TP8/PP2，D 单机为 DP2/TP8/PP1，均未开启投机解码。P 两组均开启 enable_dsa_cp，两组仅 CPP 与 SRF 开关不同；D 配置保持相同。",
        "",
