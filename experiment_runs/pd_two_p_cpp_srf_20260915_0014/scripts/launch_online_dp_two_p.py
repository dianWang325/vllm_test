...
P uses internal DP2 on P0 and two headless TP8 processes on P1. Each
DP rank spans P0/P1 as PP2 and occupies eight distinct NPUs per host.
P0 serves HTTP on 127.0.0.1:18080. D remains external DP2/TP8/PP1 on
...
def main() -> int:
    ...
    layout = LAYOUTS[args.role]
    ranks: list[list[str]] = []
    # P0 hosts both DP engines behind one API process. P1's headless
    # multiproc path starts only one PP stage, so launch each remote DP
    # stage separately with eight visible NPUs.
    ...
    for rank in range(local_processes):
        ...
        ranks.append([
            ...
            str(1 + rank * 2 if args.role == "prefill" and args.node_rank == 1
                else args.node_rank if args.node_rank is not None else -1),
