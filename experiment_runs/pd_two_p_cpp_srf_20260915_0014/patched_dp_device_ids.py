...
def _patched_get_physical_gpu_ids_for_local_dp_rank(
    ...
    # vLLM's world_size includes the cross-node PP stage. When one physical
    # host runs multiple DP ranks, local_world_size is the device count per
    # rank on that host. Slice the full local device list by this size. If
    # the caller already pre-sharded the env var, start at zero instead.
    if user_assigned_gpu_ids is None and device_control_env_var in os.environ:
        ...
        if local_dp_rank * world_size + local_world_size > len(visible):
            ...
            if start + local_world_size > len(visible):
                start = 0
            if local_world_size > len(visible):
                raise ValueError(
                    f"{device_control_env_var} exposes {len(visible)} devices, "
                    f"but this rank requires {local_world_size}"
                )
            return [
                current_platform.device_control_id_to_physical_device_id(visible[device_id])
                for device_id in range(start, start + local_world_size)
            ]
