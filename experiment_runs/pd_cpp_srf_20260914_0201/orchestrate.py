"""Run the approved eight-run A3 PD experiment on the selected two hosts."""
...
def assert_idle(host: str) -> None:
    result = ssh(host, ["npu-smi", "info"], 30)
    if result.returncode or result.stdout.count("No running processes found in NPU") != 8:
...
class Service:
    def __init__(self, name: str, host: str, command: list[str], tag: str):
        ...
        remote_command = [
            "docker", "exec", "-i", "-w", f"{REMOTE}/scripts", "wd_test0825",
...
def run_bench(tag: str, config: str, phase: str, expected: int, services: list[Service]) -> None:
    ...
    work_dir = f"{REMOTE}/results/{tag}/{phase}"
    command = [
    ...
    with log_path.open("wb") as log:
        process = subprocess.Popen([*SSH, f"root@{P_HOST}", *command], stdout=log, stderr=subprocess.STDOUT)
        deadline = time.monotonic() + (3600 if phase == "warmup" else 7200)
        try:
            while process.poll() is None:
                if any(not service.alive() for service in services):
                    process.terminate()
                    raise RuntimeError(f"A server exited during {phase}; see service logs")
...
def run_one(index: int, mode: str, dataset: str) -> None:
    ...
    assert_idle(P_HOST)
    assert_idle(D_HOST)
    snapshot(P_HOST, folder / "p-before-npu.txt")
    snapshot(D_HOST, folder / "d-before-npu.txt")
    services: list[Service] = []
    status = {
        "tag": tag, "mode": mode, "dataset": dataset,
        "p_host": P_HOST, "d_host": D_HOST,
        "status": "starting", "started_at": time.time(),
    }
    status_path.write_text(json.dumps(status, indent=2), encoding="utf-8")
    try:
        d_command = [
            PYTHON, f"{REMOTE}/scripts/launch_online_dp.py", "--role", "decode",
            "--local-ip", D_HOST, "--nic-name", "enp194s0f0",
        ]
        services.append(Service("decode", D_HOST, d_command, tag))
        p_command = [
            "env", f"PD_P_SCHEDULER_MODE={mode}", PYTHON,
            f"{REMOTE}/scripts/launch_online_dp.py", "--role", "prefill",
            "--local-ip", P_HOST, "--nic-name", "enp194s0f0",
        ]
        services.append(Service("prefill", P_HOST, p_command, tag))
        wait_for_health(services, D_HOST, [18082, 18083], "/health", 3600)
        wait_for_health(services, P_HOST, [18080], "/health", 3600)
        proxy_command = [
        ...
        services.append(Service("proxy", P_HOST, proxy_command, tag))
        wait_for_health(services, P_HOST, [18090], "/healthcheck", 300)
